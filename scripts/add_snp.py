...
from finemap_tools.others.pheweb.file_utils import (
    VariantFileReader,
    VariantFileWriter,
    get_filepath,
    read_maybe_gzip,
)
from finemap_tools.others.pheweb.utils import PheWebError
...
def get_rsid_reader(
    rsids_f: Iterator[str], rsids_filepath: str
    ...
    for line in rsids_f:
        if not line.startswith("##"):
            if line.startswith("#"):
                ...
                if len(fields) != 5:
                    raise PheWebError(
                        "Line has wrong number of fields: {!r} - {!r}".format(
                            line, fields
                        )
                    )
                chrom, pos, rsid, ref, alt_group = (
                    fields[0],
                    int(fields[1]),
                    fields[2],
                    fields[3],
                    ...
                    try:
                        ...
                        raise PheWebError(
                            (
                                ...
                                rsids_filepath,
                                chrom,
                                list(chrom_order.keys()),
                                list(chrom_aliases.keys()),
                            )
                        )
                ...
                if prev_chrom_idx > chrom_idx:
                    raise PheWebError(
                        (
                            ...
                            + "but instead {} came before {}"
                        ).format(
                            rsids_filepath,
                            chrom_order_list,
                            chrom_order_list[prev_chrom_idx],
                            chrom_order_list[chrom_idx],
                        )
                    )
                if prev_chrom_idx == chrom_idx and prev_pos > pos:
                    raise PheWebError(
                        "The rsids file, {!r}, on chromosome {!r}, has position {} before {}.".format(
                ...
                assert all(base in "ATCGN" for base in ref), (
                    chrom,
                    pos,
                    ref,
                    alt_group,
...
def get_one_chr_pos_at_a_time(
    ...
    for k, g in itertools.groupby(
        iterator, key=lambda cpra: (cpra["chrom"], cpra["pos"])
    ):
        yield list(g)
...
def args_parse():
    ...
    parser = argparse.ArgumentParser(
        ...
        description=textwrap.dedent(
            ...
            --input should have 4 columns chrom pos ref alt 
            Example:
                add_snp.py -i DNAJC16.tsv -o test_rsid.tsv --rsid ../../dbsnp/rsids-v154-hg38.tsv.gz
            """
        ),
    )
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="input sumstats, will use pandas to read the file",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="output folder, if with .gz will be compressed",
    )
    parser.add_argument(
        "--rsids",
        type=str,
        required=False,
        help="rsids file, if not given, will use the default rsids file",
    )
...
if __name__ == "__main__":
    ...
    if os.path.exists(out_filepath) and max(
        mtime(in_filepath), mtime(rsids_filepath)
    ) <= mtime(out_filepath):
        print("rsid annotation is up-to-date!")
        # raise ValueError("rsid annotation is up-to-date!")

    with VariantFileReader(in_filepath) as in_reader, read_maybe_gzip(
        rsids_filepath
    ) as rsids_f, VariantFileWriter(out_filepath, allow_extra_fields=True) as writer:
        ...
        #     rsid_group_reader = itertools.islice(
        #         rsid_group_reader, 0, debugging_limit_num_variants
        #     )
        ...
        for cp_group in cp_group_reader:
            ...
            while True:
                if rsid_group[0]["chrom"] == cp_group[0]["chrom"]:
                    ...
                    rsid_is_not_behind = (
                        chrom_order[rsid_group[0]["chrom"]]
                        >= chrom_order[cp_group[0]["chrom"]]
                    )
            ...
            if (
                rsid_group[0]["chrom"] == cp_group[0]["chrom"]
                ...
                for cpra in cp_group:
                    rsids = [
                        rsid["rsid"]
                        for rsid in rsid_group
                        if cpra["ref"] == rsid["ref"]
